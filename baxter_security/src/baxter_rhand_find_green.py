...
class GreenDetect:
	...
	def callback(self, data):
		...
		cv_imageThresh = cv2.inRange(cv_imageHSV, np.array((80, 80, 40)), np.array((90,256,256))) # Convert the HSV image to binary, getting green
		...
		# Finds the largest green shape as the lighter
		for cnt in contour:
		...
		# Once we found the largest shape we need to find its center
		if (largestArea > 0):
			...
			# Draw a line on the image so we can see the orienttaion			

...
class BlueDetect:
  ...
  def callback(self, data):
    ...
    # Look for blue
    cv_imageTestBlue = cv2.inRange(cv_imageHSV, np.array((110, 100, 40)), np.array((130,256,256)))
    cv_imageTestBlut = cv2.GaussianBlur(cv_imageThresh, (3,3), 0)
    # Look for green
    cv_imageTestGreen = cv2.inRange(cv_imageHSV, np.array((80,80,200)), np.array((90,256,256)))   
    cv_imageTestGreen = cv2.GaussianBlur(cv_imageThresh, (3,3), 0)
    # End getting color
    ...
    # Finding contours in each array
    contour, hierarchy = cv2.findContours(cv_imageTestBlue, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    greenContour, greenHierarchy = cv2.findContours(cv_imageTestGreen, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
  
    contour.extend(greenContour)
